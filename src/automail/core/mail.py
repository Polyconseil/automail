...
class Codec(object):
    ...
    def get_id(cls, item):
        ...
        return getattr(
            item,
            'id',
            hashlib.sha3_256(str(item).encode('utf-8')).hexdigest()[:32]
        )
...
def create(item, codec=Codec):
    ...
    message.add_header('Reply-To', '{}+{}@{}'.format(
        sender, identifier, domain))
    message.add_header(
        'Subject',
        ''.join(
            '[{}]'.format(t)
...
def parse(data, codec=Codec):
    ...
    new = bool(new_tag)
    external_id, subject = utils_parse.consume_re(
        codec.re_external_id(),
        subject
    )
